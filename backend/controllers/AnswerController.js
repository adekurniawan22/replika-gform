...
class AnswerController{
    async store(req,res){
        try{
            if(!req.params.formId) { throw { code: 428, message: "REQUIRED_FORM_ID" } }
            if(!mongoose.Types.ObjectId.isValid(req.params.formId)) { throw { code: 400, message: "INVALID_FORM_ID" } }

            const form = await Form.findById(req.params.formId);

            const isDuplicate  = await answerDuplicate(req.body.answers)
            if(isDuplicate){ throw { code: 404, message: "DUPLICATE_ANSWER" } }

            const isQuestionRequiredButEmpty =  await questionRequiredButEmpty(form, req.body.answers)
            if(isQuestionRequiredButEmpty){ throw { code: 404, message: "QUESTION_REQUIRED_BUT_EMPTY" } }

            const isOptionValueNotExist =  await optionValueNotExist(form, req.body.answers)
            if(isOptionValueNotExist){ throw { code: 404, message: "INVALID_VALUE_QUESTION" } }
            ...
            if(!answer) { throw { code: 500, message: "ADD_ANSWER_FAILED" } }
